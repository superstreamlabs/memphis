...
func (s *Server) updateStationProducersOfSchemaChange(tenantName string, sn StationName, schemaUpdate models.ProducerSchemaUpdate) {
	...
	if err != nil {
		s.Errorf("[tenant name: %v]updateStationProducersOfSchemaChange: marshal failed at station %v", tenantName, sn.external)
		return
	}

	account, err := s.lookupAccount(tenantName)
	if err != nil {
		s.Errorf("[tenant name: %v]updateStationProducersOfSchemaChange %v", tenantName, err.Error())
		return
	}
...
func getSchemaByStationName(sn StationName, tenantName string) (models.Schema, error) {
	...
	if err != nil {
		serv.Errorf("[tenant name: %v]getSchemaByStation: At station %v: %v", tenantName, sn.external, err.Error())
		return models.Schema{}, err
	}
	if !exist {
		errMsg := fmt.Sprintf("[tenant name: %v]Station %v does not exist", tenantName, station.Name)
	...
	if err != nil {
		serv.Errorf("[tenant name: %v]getSchemaByStation: Schema %v at station %v: %v", tenantName, station.SchemaName, station.Name, err.Error())
		return models.Schema{}, err
	}
	if !exist {
		serv.Warnf("[tenant name: %v]getSchemaByStation: Schema %v does not exist", tenantName, station.SchemaName)
		return models.Schema{}, ErrNoSchema
	}
...
func (sh SchemasHandler) CreateNewSchema(c *gin.Context) {
	...
	if err != nil {
		serv.Warnf("CreateNewSchema: %v", err.Error())
		c.AbortWithStatusJSON(SHOWABLE_ERROR_STATUS_CODE, gin.H{"message": err.Error()})
		return
	}
	user, err := getUserDetailsFromMiddleware(c)
	if err != nil {
		serv.Errorf("CreateNewSchema: Schema %v: %v", schemaName, err.Error())
	...
	if err != nil {
		serv.Errorf("[tenant name: %v][user name: %v]CreateNewSchema: Schema %v: %v", user.TenantName, user.Username, schemaName, err.Error())
		c.AbortWithStatusJSON(500, gin.H{"message": "Server Error"})
		return
	}
	if exist {
		errMsg := fmt.Sprintf("Schema with the name %v already exists", schemaName)
		serv.Warnf("[tenant name: %v][user name: %v]CreateNewSchema: %v", user.TenantName, user.Username, errMsg)
	...
	if err != nil {
		serv.Warnf("[tenant name: %v][user name: %v]CreateNewSchema: Schema %v: %v", user.TenantName, user.Username, schemaName, err.Error())
		c.AbortWithStatusJSON(SHOWABLE_ERROR_STATUS_CODE, gin.H{"message": err.Error()})
		return
	}
	...
	if rowsUpdated == 1 {
		...
		if err != nil {
			serv.Errorf("[tenant name: %v][user name: %v]CreateNewSchema: %v", user.TenantName, user.Username, err.Error())
			c.AbortWithStatusJSON(500, gin.H{"message": "Server error"})
			return
		}
		message := fmt.Sprintf("[tenant name: %v][user name: %v]Schema %v has been created by %v", user.TenantName, user.Username, schemaName, user.Username)
	...
	if len(body.Tags) > 0 {
		...
		if err != nil {
			serv.Errorf("[tenant name: %v][user name: %v]CreateNewSchema: Failed creating tag at schema %v: %v", user.TenantName, user.Username, schemaName, err.Error())
			c.AbortWithStatusJSON(500, gin.H{"message": "Server error"})
			return
		}
	}
...
func (sh SchemasHandler) GetAllSchemas(c *gin.Context) {
	...
	if err != nil {
		serv.Errorf("[tenant name: %v][user name: %v]GetAllSchemas: %v", user.TenantName, user.Username, err.Error())
		c.AbortWithStatusJSON(500, gin.H{"message": "Server error"})
		return
	}
...
func (sh SchemasHandler) GetSchemaDetails(c *gin.Context) {
	...
	user, err := getUserDetailsFromMiddleware(c)
	if err != nil {
		serv.Errorf("GetSchemaDetails: Schema %v: %v", schemaName, err.Error())
		c.AbortWithStatusJSON(500, gin.H{"message": "Server error"})
		return
	}
	exist, schema, err := db.GetSchemaByName(schemaName, user.TenantName)
	if err != nil {
		serv.Errorf("[tenant name: %v][user name: %v]GetSchemaDetails: Schema %v: %v", user.TenantName, user.Username, body.SchemaName, err.Error())
		c.AbortWithStatusJSON(500, gin.H{"message": "Server error"})
		return
	}
	if !exist {
		errMsg := fmt.Sprintf("Schema %v does not exist", body.SchemaName)
		serv.Warnf("[tenant name: %v][user name: %v]GetSchemaDetails: %v", user.TenantName, user.Username, errMsg)
		c.AbortWithStatusJSON(SHOWABLE_ERROR_STATUS_CODE, gin.H{"message": errMsg})
		return
	}
	...
	if err != nil {
		serv.Errorf("[tenant name: %v][user name: %v]GetSchemaDetails: Schema %v: %v", user.TenantName, user.Username, schemaName, err.Error())
		c.AbortWithStatusJSON(500, gin.H{"message": "Server error"})
		return
	}
...
func deleteSchemaFromStations(s *Server, schemaName string, tenantName string) error {
	...
	if err != nil {
		s.Errorf("[tenant name: %v]deleteSchemaFromStations: Schema %v: %v", tenantName, schemaName, err.Error())
		return err
	}
...
func (sh SchemasHandler) RemoveSchema(c *gin.Context) {
	...
	for _, name := range body.SchemaNames {
		...
		if err != nil {
			serv.Errorf("[tenant name: %v][user name: %v]RemoveSchema: Schema %v: %v", user.TenantName, user.Username, schemaName, err.Error())
			c.AbortWithStatusJSON(500, gin.H{"message": "Server error"})
			return
		}
		if exist {
	...
	if len(schemaIds) > 0 {
		...
		if err != nil {
			serv.Errorf("[tenant name: %v][user name: %v]RemoveSchema: Schema %v", user.TenantName, user.Username, err.Error())
			c.AbortWithStatusJSON(500, gin.H{"message": "Server error"})
			return
		}
		for _, name := range body.SchemaNames {
			serv.Noticef("[tenant name: %v][user name: %v]Schema %v has been deleted", user.TenantName, user.Username, name)
		}
	}
...
func (sh SchemasHandler) CreateNewVersion(c *gin.Context) {
	...
	user, err := getUserDetailsFromMiddleware(c)
	if err != nil {
		serv.Errorf("CreateNewVersion: Schema %v: %v", body.SchemaName, err.Error())
		c.AbortWithStatusJSON(401, gin.H{"message": "Unauthorized"})
		return
	}
	schemaName := strings.ToLower(body.SchemaName)
	exist, schema, err := db.GetSchemaByName(schemaName, user.TenantName)
	if err != nil {
		serv.Errorf("[tenant name: %v][user name: %v]CreateNewVersion: Schema %v: %v", user.TenantName, user.Username, body.SchemaName, err.Error())
		c.AbortWithStatusJSON(500, gin.H{"message": "Server Error"})
		return
	}
	if !exist {
		errMsg := fmt.Sprintf("Schema %v does not exist", body.SchemaName)
		serv.Warnf("[tenant name: %v][user name: %v]CreateNewVersion: %v", user.TenantName, user.Username, errMsg)
		c.AbortWithStatusJSON(SHOWABLE_ERROR_STATUS_CODE, gin.H{"message": errMsg})
		return
	}
	...
	if schema.Type == "protobuf" {
		...
		if err != nil {
			serv.Warnf("[tenant name: %v][user name: %v]CreateNewVersion: Schema %v: %v", user.TenantName, user.Username, body.SchemaName, err.Error())
			c.AbortWithStatusJSON(SCHEMA_VALIDATION_ERROR_STATUS_CODE, gin.H{"message": err.Error()})
			return
		}
	}
	...
	if err != nil {
		serv.Warnf("[tenant name: %v][user name: %v]CreateNewVersion: %v", user.TenantName, user.Username, err.Error())
		c.AbortWithStatusJSON(SCHEMA_VALIDATION_ERROR_STATUS_CODE, gin.H{"message": err.Error()})
		return
	}
	if rowsUpdated == 1 {
		serv.Noticef("[tenant name: %v][user name: %v]Schema Version %v has been created by %v", user.TenantName, user.Username, strconv.Itoa(newSchemaVersion.VersionNumber), user.Username)
	} else {
		serv.Warnf("tenant name: %v][user name: %v]CreateNewVersion: Schema %v: Version %v already exists", user.TenantName, user.Username, body.SchemaName, strconv.Itoa(newSchemaVersion.VersionNumber))
...
func (sh SchemasHandler) RollBackVersion(c *gin.Context) {
	...
	user, err := getUserDetailsFromMiddleware(c)
	if err != nil {
		serv.Errorf("RollBackVersion: Schema %v: %v", body.SchemaName, err.Error())
		c.AbortWithStatusJSON(500, gin.H{"message": "Server Error"})
		return
	}
	schemaName := strings.ToLower(body.SchemaName)
	exist, schema, err := db.GetSchemaByName(schemaName, user.TenantName)
	if err != nil {
		serv.Errorf("[tenant name: %v][user name: %v]RollBackVersion: Schema %v: %v", user.TenantName, user.Username, body.SchemaName, err.Error())
		c.AbortWithStatusJSON(500, gin.H{"message": "Server Error"})
		return
	}
	if !exist {
		errMsg := fmt.Sprintf("Schema %v does not exist", body.SchemaName)
		serv.Warnf("[tenant name: %v][user name: %v]RollBackVersion: %v", user.TenantName, user.Username, errMsg)
		c.AbortWithStatusJSON(SHOWABLE_ERROR_STATUS_CODE, gin.H{"message": errMsg})
		return
	}
	...
	if err != nil {
		serv.Errorf("[tenant name: %v][user name: %v]RollBackVersion: Schema %v version %v: %v", user.TenantName, user.Username, body.SchemaName, strconv.Itoa(schemaVersion), err.Error())
		c.AbortWithStatusJSON(500, gin.H{"message": "Server error"})
		return
	}
	if !exist {
...
func (sh SchemasHandler) ValidateSchema(c *gin.Context) {
	...
	err := validateSchemaType(schemaType)
	if err != nil {
		serv.Warnf("ValidateSchema: Schema type %v: %v", schemaType, err.Error())
		c.AbortWithStatusJSON(SHOWABLE_ERROR_STATUS_CODE, gin.H{"message": err.Error()})
		return
	}
...
func (s *Server) createSchemaDirect(c *client, reply string, msg []byte) {
	...
	if err != nil {
		s.Errorf("[tenant name: %v]createSchemaDirect - failed creating Schema: %v", tenantName, err.Error())
	...
	if err != nil {
		s.Warnf("[tenant name: %v]createSchemaDirect - Schema is not in the right %v format, error: %v", tenantName, csr.Type, err.Error())
		respondWithRespErr(tenantName, s, reply, err, &resp)
		return
	}
	...
	if csr.Type == "protobuf" {
		...
		if err != nil {
			s.Errorf("[tenant name: %v]createSchemaDirect - failed creating Schema: %v : %v", tenantName, csr.Name, err.Error())
			respondWithRespErr(tenantName, s, reply, err, &resp)
		}
	}
	...
	if csr.Type == "protobuf" {
		...
		if err != nil {
			s.Warnf("[tenant name: %v]createSchemaDirect - failed creating Schema: %v : %v", tenantName, csr.Name, err.Error())
			respondWithRespErr(tenantName, s, reply, err, &resp)
			return
		}
	}
	...
	if exist {
		if existedSchema.Type == csr.Type {
			...
			s.Warnf("[tenant name: %v]createSchemaDirect: %v Bad Schema Type", tenantName, csr.Name)
...
func (s *Server) updateSchemaVersion(schemaID int, tenantName string, newSchemaReq CreateSchemaReq) error {
	...
	if err != nil {
		s.Errorf("[tenant name: %v]updateSchemaVersion: Schema %v: %v", tenantName, newSchemaReq.Name, err.Error())
		return err
	}

	countVersions, err := db.GetShcemaVersionsCount(schemaID, user.TenantName)
	if err != nil {
		s.Errorf("[tenant name: %v][user name: %v]updateSchemaVersion: Schema %v: %v", tenantName, user.Username, newSchemaReq.Name, err.Error())
		return err
	}
	...
	if newSchemaReq.Type == "protobuf" {
		...
		if err != nil {
			s.Errorf("[tenant name: %v][user name: %v]CreateNewSchemaDirectn: could not create proto descriptor for %v: %v", tenantName, user.Username, newSchemaReq.Name, err.Error())
			return err
		}
	}
	...
	if err != nil {
		s.Errorf("[tenant name: %v][user name: %v]updateSchemaVersion: %v", tenantName, user.Username, err.Error())
		return err
	}
	if rowsUpdated == 1 {
		message := fmt.Sprintf("[tenant name: %v][user name: %v]Schema Version %v has been created by %v", tenantName, user.Username, strconv.Itoa(newSchemaVersion.VersionNumber), user.Username)
		s.Noticef(message)
		return nil
	} else {
		s.Errorf("[tenant name: %v][user name: %v]updateSchemaVersion: schema update failed", tenantName, user.Username)
...
func (s *Server) createNewSchema(newSchemaReq CreateSchemaReq, tenantName string) error {
	...
	_, user, err := db.GetUserByUsername(newSchemaReq.CreatedByUsername, tenantName)
	if err != nil {
		s.Errorf("[tenant name: %v]createNewSchema: %v", tenantName, err.Error())
		return err
	}
	...
	if newSchemaReq.Type == "protobuf" {
		descriptor, err = generateSchemaDescriptor(newSchemaReq.Name, 1, newSchemaReq.SchemaContent, newSchemaReq.Type)
		if err != nil {
			s.Errorf("[tenant name: %v][user name: %v]CreateNewSchema: Schema %v: %v", tenantName, user.Username, newSchemaReq.Name, err.Error())
			return err
		}
	}
	...
	if err != nil {
		s.Errorf("[tenant name: %v][user name: %v]createNewSchema: %v", tenantName, user.Username, err.Error())
		return err
	}
