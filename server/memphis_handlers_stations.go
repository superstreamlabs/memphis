...
func (s *Server) createStationDirectIntern(c *client,
	...
	if shouldCreateStream {
		err = s.CreateStream(stationName, retentionType, retentionValue, storageType, csr.IdempotencyWindow, replicas, csr.TieredStorageEnabled, csr.DedupEnabled)
		if err != nil {
...
func (sh StationsHandler) CreateStation(c *gin.Context) {
	...
	err = sh.S.CreateStream(stationName, retentionType, body.RetentionValue, body.StorageType, body.IdempotencyWindow, body.Replicas, body.TieredStorageEnabled, body.DedupEnabled)
	if err != nil {
		if IsNatsErr(err, JSInsufficientResourcesErr) {
