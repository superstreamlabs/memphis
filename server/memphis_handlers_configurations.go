...
func changeDlsRetention(dlsRetention int) error {
	err := db.UpsertConfiguration("dls_retention", strconv.Itoa(dlsRetention), strings.ToLower(db.GlobalTenant))
	if err != nil {
		return err
	}
	return nil
...
func changeLogsRetention(logsRetention int) error {
	err := db.UpsertConfiguration("logs_retention", strconv.Itoa(logsRetention), strings.ToLower(db.GlobalTenant))
	if err != nil {
		return err
	}
...
func changeTSTime(tsTime int) error {
	err := db.UpsertConfiguration("tiered_storage_time_sec", strconv.Itoa(tsTime), strings.ToLower(db.GlobalTenant))
	if err != nil {
		return err
	}
...
func EditClusterCompHost(key string, host string) error {
	...
	err := db.UpsertConfiguration(key, host, strings.ToLower(db.GlobalTenant))
	if err != nil {
		return err
	}
...
func changeMaxMsgSize(newSize int) error {
	err := db.UpsertConfiguration("max_msg_size_mb", strconv.Itoa(newSize), strings.ToLower(db.GlobalTenant))
	if err != nil {
		return err
	}
