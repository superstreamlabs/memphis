...
func changePMRetention(pmRetention int) error {
	...
	err = db.UdateConfiguration("pm_retention", strconv.Itoa(POISON_MSGS_RETENTION_IN_HOURS))
	if err != nil {
		return err
	}
...
func changeLogsRetention(logsRetention int) error {
	LOGS_RETENTION_IN_DAYS = logsRetention
	err := db.UdateConfiguration("logs_retention", strconv.Itoa(LOGS_RETENTION_IN_DAYS))
	if err != nil {
		return err
	}
...
func changeTSTime(tsTime int) error {
	...
	err = db.UdateConfiguration("tiered_storage_time_sec", strconv.Itoa(TIERED_STORAGE_TIME_FRAME_SEC))
	if err != nil {
		return err
	}
...
func editClusterCompHost(key string, host string) error {
	...
	err = db.UdateConfiguration(key, host)
	if err != nil {
		return err
	}
	return nil
