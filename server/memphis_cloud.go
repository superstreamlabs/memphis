...
func (mh MonitoringHandler) GetSystemLogs(c *gin.Context) {
	...
	if logSource == "" {
		serv.Warnf("GetSystemLogs: log_source should be one of the following: empty or memphis-{number between 1 to amount of clusters}")
		c.AbortWithStatusJSON(SHOWABLE_ERROR_STATUS_CODE, gin.H{"message": "log_source should be one of the following: empty or memphis-{number between 1 to amount of clusters}"})
		return
	}
