...
func TestMemphisGetMsgs(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			msgsToFetchNum := 1
			memphisMsgs, err := s.memphisGetMsgs(MEMPHIS_GLOBAL_ACCOUNT, "", mset.name(), 1, msgsToFetchNum, 1*time.Second, true)
			if err != nil {
			...
			msgsToFetchNum = 3
			_, err = s.memphisGetMsgs(MEMPHIS_GLOBAL_ACCOUNT, "", mset.name(), 1, msgsToFetchNum, 1*time.Second, true)
			if err != ErrStoreEOF {
				t.Fatalf("Unexpected error getting messages: %v", err)
			}
