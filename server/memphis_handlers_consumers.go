...
func (s *Server) createConsumerDirectCommon(c *client, consumerName, cStationName, cGroup, cType, connectionId string, maxAckTime, maxMsgDeliveries, requestVersion int, startConsumeFromSequence uint64, lastMessages int64) error {
	...
	// if !exist {
	// 	var created bool
	_, _, err = CreateDefaultStation(s, stationName, connection.CreatedByUser)
	if err != nil {
	...
	newConsumer, rowsUpdated, err := db.UpsertNewConsumerV1(name, stationId, consumerType, connectionId, 1, consumerGroup, maxAckTime, maxMsgDeliveries, startConsumeFromSequence, lastMessages)
	if err != nil {
		errMsg := "Consumer " + consumerName + " at station " + cStationName + ": " + err.Error()
		serv.Errorf("createConsumerDirectCommon: " + errMsg)
		return err
	...
	if rowsUpdated == 1 {
		...
		if consumerGroupExist {
			if requestVersion == 1 {
				if newConsumer.StartConsumeFromSeq != consumerFromGroup.StartConsumeFromSequence || newConsumer.LastMessages != consumerFromGroup.LastMessages {
