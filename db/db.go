...
func GetAllConsumersByStation(stationId int) ([]models.ExtendedConsumer, error) {
	...
	defer conn.Release()
	query := `SELECT DISTINCT ON (c.name, c.consumers_group) c.id, c.name, c.updated_at, c.is_active, c.connection_id, c.consumers_group, c.max_ack_time_ms, c.max_msg_deliveries, s.name,
