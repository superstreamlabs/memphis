...
func createTables(MetadataDbClient MetadataStorage) error {
	...
	BEGIN
		IF EXISTS (
			...
			ALTER TABLE audit_logs ADD COLUMN IF NOT EXISTS tenant_name VARCHAR NOT NULL DEFAULT '$G';
	...
	BEGIN
		...
		ALTER TABLE connections ADD COLUMN IF NOT EXISTS tenant_name VARCHAR NOT NULL DEFAULT '$G';
		END IF;
	END $$;`
	...
	BEGIN
		...
		ALTER TABLE consumers ADD COLUMN IF NOT EXISTS tenant_name VARCHAR NOT NULL DEFAULT '$G';
	...
	BEGIN
		...
		ALTER TABLE stations ADD COLUMN IF NOT EXISTS tenant_name VARCHAR NOT NULL DEFAULT '$G';
	...
	BEGIN
		...
		ALTER TABLE schema_versions ADD COLUMN IF NOT EXISTS tenant_name VARCHAR NOT NULL DEFAULT '$G';
		END IF;
	END $$;`
	...
	BEGIN
		...
		ALTER TABLE producers ADD COLUMN IF NOT EXISTS tenant_name VARCHAR NOT NULL DEFAULT '$G';
	...
	BEGIN
		...
		ALTER TABLE dls_messages ADD COLUMN IF NOT EXISTS tenant_name VARCHAR NOT NULL DEFAULT '$G';
		END IF;
	END $$;`
...
func RemoveConnectionsOfDeletedUsers(tenantNames string) error {
	ctx, cancelfunc := context.WithTimeout(context.Background(), DbOperationTimeout*time.Second)
	defer cancelfunc()
	conn, err := MetadataDbClient.Client.Acquire(ctx)
	if err != nil {
	...
	_, err = conn.Conn().Exec(ctx, stmt.Name, tenantNames)
	if err != nil {
		return err
	}
	return nil
...
func RemoveAuditLogsOfDeletedUsers(tenantName string) error {
	ctx, cancelfunc := context.WithTimeout(context.Background(), DbOperationTimeout*time.Second)
	defer cancelfunc()
	conn, err := MetadataDbClient.Client.Acquire(ctx)
	if err != nil {
...
func RemoveStationsOfDeletedUsers(tenantName string) error {
	ctx, cancelfunc := context.WithTimeout(context.Background(), DbOperationTimeout*time.Second)
	defer cancelfunc()
	conn, err := MetadataDbClient.Client.Acquire(ctx)
	if err != nil {
...
func RemoveProducersOfDeletedUsers(tenantName string) error {
	ctx, cancelfunc := context.WithTimeout(context.Background(), DbOperationTimeout*time.Second)
	defer cancelfunc()
	conn, err := MetadataDbClient.Client.Acquire(ctx)
	if err != nil {
...
func RemoveConsumersOfDeletedUsers(tenantName string) error {
	ctx, cancelfunc := context.WithTimeout(context.Background(), DbOperationTimeout*time.Second)
	defer cancelfunc()
	conn, err := MetadataDbClient.Client.Acquire(ctx)
	if err != nil {
...
func RemoveSchemasOfDeletedUsers(tenantName string) error {
	ctx, cancelfunc := context.WithTimeout(context.Background(), DbOperationTimeout*time.Second)
	defer cancelfunc()
	conn, err := MetadataDbClient.Client.Acquire(ctx)
	if err != nil {
...
func RemoveSchemaVersionsOfDeletedUsers(tenantName string) error {
	ctx, cancelfunc := context.WithTimeout(context.Background(), DbOperationTimeout*time.Second)
	defer cancelfunc()
	conn, err := MetadataDbClient.Client.Acquire(ctx)
	if err != nil {
...
func DeleteUsersPerTenant(tenantName string) error {
	ctx, cancelfunc := context.WithTimeout(context.Background(), DbOperationTimeout*time.Second)
	defer cancelfunc()
...
func RemoveTagsResources(tenantName string) error {
	...
	stmt, err := conn.Conn().Prepare(ctx, "remove_tenant_resources", query)
	if err != nil {
		return err
	}
