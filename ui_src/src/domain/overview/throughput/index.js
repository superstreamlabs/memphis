...
//     const [componentLocation, setComponentLocation] = useState(0);
//     const [readWriteLocation, setReadWriteLocation] = useState(0);
...
//     useEffect(() => {
//         if (!data && state?.monitor_data?.brokers_throughput?.length > 0) initiateDataState();
//         else {
//             let updatedDataSets = data.datasets;
//             // console.log(updatedDataSets);
//             state?.monitor_data?.brokers_throughput?.forEach((sample, i) => {
//                 // console.log(sample?.read?.map((item) => item.read));
//                 updatedDataSets[i * 2].data = [...updatedDataSets[i * 2].data, ...sample?.write?.map((item) => item.write)];
//                 updatedDataSets[i * 2 + 1].data = [...updatedDataSets[i * 2 + 1].data, ...sample?.read?.map((item) => item.read)];
//             });
//             // console.log(updatedDataSets);
...
//         let updatedDataSamples = { ...data.datasets };
//         let value;
//         const location = type === 'write' ? componentLocation * 2 : componentLocation * 2 + 1;
//         value = data.datasets[location]?.data[0];
//         // updatedDataSamples = updatedDataSamples[location]?.data[0].shift();
//         // setData({ ...data, datasets: updatedDataSamples });
//         return value;
//     };
